...
const FAQ = () => {
  ...
  const [movingQuestion, setMovingQuestion] = useState<number | null>(null);
  ...
  const handleTabChange = (index: number) => {
    ...
    setTimeout(() => {
      ...
      }, 200); // Slightly faster animation for middle panel
    }, 500);
  };
  ...
  // Get animation style for the specific question that's moving
  const getQuestionStyle = (index: number): React.CSSProperties => {
    if (!animating) return {};
    
    // If the clicked question is from the right panel
    if (direction === 'next' && movingQuestion !== null && index <= movingQuestion && index > activeTab) {
      // These questions are moving from right to left
      return {
        position: 'relative' as const,
        zIndex: 10,
        transform: `translateX(-${middlePanelWidth}px)`,
        transition: 'transform 500ms ease-in-out',
        background: '#A7CAED'  // Keep solid background while moving
      };
    }
    
    // If the clicked question is from the left panel
    if (direction === 'prev' && movingQuestion !== null && index >= movingQuestion && index < activeTab) {
      // These questions are moving from left to right
      return {
        position: 'relative' as const,
        zIndex: 10,
        transform: `translateX(${middlePanelWidth}px)`,
  ...
  // Get animation style for content
        ...
        <div className="bg-white rounded-xl overflow-hidden shadow-lg border-[#8EBAE3] border-[1px]">
          ...
          <div className="md:hidden flex flex-col">
            ...
            <div className="w-full p-6 bg-white relative min-h-[400px] overflow-hidden">
              ...
              <div style={getContentAnimationStyle()}>
              ...
              {/* Mascot image */}
              <div className="absolute top-4 right-0 w-20 h-20 md:w-32 md:h-32">
                <Image 
